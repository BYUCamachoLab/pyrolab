...
os.environ["PATH"] = (
    "C:\\Program Files\\SacherLasertechnik\\MotorMotion 2.1\\data"
    + ";"
    + os.environ["PATH"]
)
...
def wl_to_du(wl):
    if wl < WL_MIN:
        ...
        wl = WL_MAX
    du = -1 * B_COEF / (2 * A_COEF) - math.sqrt(
        (wl - C_COEF) / A_COEF + math.pow((B_COEF / (2 * A_COEF)), 2)
    )
    return round(du)
...
ret = epm.VCS_SetPositionProfile(
    keyhandle, NodeID, c_dword(vel), c_dword(acc), c_dword(acc), byref(pErrorCode)
)

ret = epm.VCS_MoveToPosition(
    keyhandle, NodeID, wl_to_du(wl_low), 1, 0, byref(pErrorCode)
)
...
comp = False
while comp == False:
    ret = epm.VCS_GetMovementState(
        keyhandle, NodeID, byref(pTargetReached), byref(pErrorCode)
    )
...
while sweep_done == False:
    ret = epm.VCS_MoveToPosition(
        keyhandle, NodeID, wl_to_du(wl_high), 1, 0, byref(pErrorCode)
    )
    comp = False
    while comp == False:
        ret = epm.VCS_GetMovementState(
        ...
        if time.time() - init_time > 1:
            init_time = time.time()
            epm.VCS_GetPositionIs(
                keyhandle, NodeID, byref(pPositionIs), byref(pErrorCode)
            )
